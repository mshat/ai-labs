...
from tag_condition import (AndMultiTagCondition, OrMultiTagCondition, AndTagCondition, OrTagCondition, TagCondition,
                           MultiTagCondition, PatternMatcher)
...
class TestQueryPatternWithSimpleConditions(unittest.TestCase):
    query_mock = Mock(query_tag_structure={'search': None}, arguments={'ArtistArgument': None})

    def test_one_AND_condition(self):
        condition = AndTagCondition('search')
        pattern_matcher = PatternMatcher(conditions=[condition])

        res = pattern_matcher.match_pattern(self.query_mock.query_tag_structure)
    ...
    def test_two_AND_conditions_pos(self):
        ...
        self.query_mock.query_tag_structure = {'search': None, 'artist': None}
    ...
    def test_two_AND_conditions_neg_no_tag_artist_in_query(self):
        ...
        condition1 = AndTagCondition('search')
        condition2 = AndTagCondition('artist')
        self.query_mock.query_tag_structure = {'search': None}
        pattern_matcher = PatternMatcher(conditions=[condition1, condition2])
...
class TestQueryPatternWithMultiConditions(unittest.TestCase):
    query_mock = Mock(query_tag_structure={'search': None, 'artist': None}, arguments={'ArtistArgument': None})
